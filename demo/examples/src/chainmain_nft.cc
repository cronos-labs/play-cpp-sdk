...
int main(int argc, char *argv[]) {
    ...
    auto schema = R""""(
  {
    ...
    signedtx = get_nft_transfer_signed_tx(tx_info, *signer2_privatekey,
                                          token_id, denom_id, from);
    resp = broadcast_tx(myservertendermint, signedtx).tx_hash_hex;
    ...
    signer1_sn += 1;
    tx_info.sequence_number = signer1_sn;
    signedtx = get_nft_burn_signed_tx(tx_info, *signer1_privatekey, token_id,
                                      denom_id);
    resp = broadcast_tx(myservertendermint, signedtx).tx_hash_hex;
